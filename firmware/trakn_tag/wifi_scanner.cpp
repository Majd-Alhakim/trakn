...
// ---------------------------------------------------------------------------
bool wifiScanComplete() { return _scanReady; }
...
void wifiCollectResults(WifiEntry results[MAX_SCAN_RESULTS], int &count) {
  ...
  for (int16_t i = MAX_SCAN_RESULTS; i < total; i++) {
    ...
    for (int k = 1; k < kept; k++) {
      if (WiFi.RSSI(indices[k]) < WiFi.RSSI(indices[weakIdx]))
        weakIdx = k;
    }
  ...
  for (int k = 0; k < kept; k++) {
    ...
    // BSSID — Ameba BSSID(uint8_t*) writes into a caller-supplied 6-byte buffer
    uint8_t mac[6] = {};
    WiFi.BSSID(mac);
    snprintf(results[k].bssid, 18, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0],
